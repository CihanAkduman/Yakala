...
/**
 * Yeni vs mevcut müşteri widget'ı.
 */
function NewVsReturningWidget() {
	const theme = useTheme();
	const { data: widget } = useGetWidget<NewVsReturningWidgetType>('Müşteri');
	...
	return (
		<Paper className="flex flex-auto flex-col overflow-hidden rounded-xl p-4 shadow-sm">
			<div className="flex flex-col items-start justify-between sm:flex-row">
				<Typography className="truncate text-lg leading-6 font-medium tracking-tight">Yeni vs Mevcut</Typography>
				<div className="ml-2">
					<Chip
						size="small"
					...
					{series.map((dataset, i) => (
						...
						>
							...
							<Typography className="text-right font-medium">
								{((uniqueVisitors * dataset) / 100).toLocaleString('tr-TR')}
							</Typography>
							<Typography
