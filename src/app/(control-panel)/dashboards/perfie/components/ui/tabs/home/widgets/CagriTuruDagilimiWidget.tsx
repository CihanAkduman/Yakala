...
/**
 * The CagriTuruDagilimiWidget widget.
 */
function CagriTuruDagilimiWidget() {
	// Mock data
	const widget = {
		overview: {
			'bu-hafta': {
		...
		series: {
			'bu-hafta': [26, 32, 28, 37, 18, 15, 41],
			'gecen-hafta': [18, 30, 25, 28, 16, 19, 37]
		},
		labels: ['CAC', 'Rezervasyon', 'Bilet', 'Check-in', 'Benefits', 'Bilet Değişikliği', 'Satış'],
		ranges: {
			'bu-hafta': 'Bu Hafta',
		...
		<Paper className="flex h-full flex-auto flex-col overflow-hidden rounded-xl p-6 shadow-sm">
			...
			>
				<div className="flex flex-col items-center justify-center p-6 sm:p-8">
					...
					<Typography className="text-secondary mt-1 text-center">Gelen Çağrılar</Typography>
				</div>
				<div className="flex flex-col items-center justify-center p-1.5 sm:p-2">
					<div className="text-5xl leading-none font-semibold tracking-tighter">
						{overview[currentRange].completed}
					</div>
					<Typography className="text-secondary mt-1 text-center">Tamamlanan Çağrılar</Typography>
				</div>
			</Box>
		</Paper>
	);
...
export default memo(CagriTuruDagilimiWidget);