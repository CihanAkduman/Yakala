...
const tasksSchema = z.object({
	id: z.string().nonempty('ID gerekli'),
	content: z.string().nonempty('İçerik gerekli'),
	completed: z.boolean()
});
...
const schema = z.object({
	...
	oneOfThemRequired: z
		...
		.refine(
			...
			{
				message: 'Alanlardan en az biri gerekli.'
			}
		)
});
					...
					{(watchedNoteForm.labels || watchedNoteForm.reminder || watchedNoteForm.createdAt) && (
						<div className="-mx-1 mb-3 flex w-full flex-wrap px-4">
							...
							{watchedNoteForm.createdAt && (
								...
								>
									Düzenlendi: {format(new Date(watchedNoteForm.createdAt), 'MMM dd yy, h:mm')}
								</Typography>
							)}
						</div>
					)}
			...
			<div className="flex flex-auto items-center justify-between px-4 pb-3">
				<div className="flex items-center">
					...
					<Controller
						...
						render={({ field: { onChange, value } }) => (
							<Tooltip
								title={value ? 'Arşivden Çıkar' : 'Arşivle'}
								placement="bottom"
							>
								<IconButton
